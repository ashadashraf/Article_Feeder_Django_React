...
function AddArticle() {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://127.0.0.1:8000/article/create/article/', formDataToSend, {
                headers: {
            ...
            if (error.response) {
                console.error("An error occurred while creating the article. Server responded with:", error.response.status);
        ...
        <div className="bg-primary p-4" style={{maxWidth: "600px"}} >
            <Form onSubmit={handleFormSubmit}>
                ...
                <Form.Group as={Row} className="mb-3 d-flex justify-content-between" controlId="formImage">
                    ...
                    <Col sm="8">
                        <Form.Control type="file" name='image' accept='image/*' required onChange={handleFileChange} />
                    </Col>
                </Form.Group>
